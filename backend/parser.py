...
from backend.provider import get_active_provider, get_vlm_provider
from backend.shared import get_shared_client, STORAGE_DIR, WORKSPACE_DIR, IMAGES_DIR

logger = logging.getLogger(__name__)

def _run_async(coro):
    """安全地运行异步协程，兼容 FastAPI 事件循环和独立脚本两种场景。
    
    当检测到已有事件循环时（FastAPI/Uvicorn），在新线程中执行 asyncio.run()。
    当没有事件循环时（脚本/测试），直接 asyncio.run()。
    参考: PageIndex/pageindex/client.py L109-L114
    """
    try:
        asyncio.get_running_loop()
        # 已有事件循环运行中 → 在新线程中执行，避免 RuntimeError
        ...
        if provider.provider_type == "ollama":
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": "Describe this document page, including all text, layout, tables, and images/charts in detail. Output in structured Markdown.",
                        "images": [img_b64]
                    }
                ],
                "stream": False,
                ...
                messages=[
                    {
                        "role": "user",
                        "content": [
                            {"type": "text", "text": "Describe this document page, including all text, layout, tables, and images/charts in detail. Output in structured Markdown."},
...
def parse_image(file_path, doc_id):
    ...
    description = _call_vlm(cached_img_path)
    if not description:
...
def parse_pdf_hybrid(file_path, doc_id):
    ...
    for i in range(len(doc)):
        ...
        if not raw_text or len(image_list) > 0:
            ...
            # VLM call
            vlm_text = _call_vlm(cached_img_path)
            
            if raw_text:
                if vlm_text:
                    page_text = raw_text + "\n\n### [Visual Layout & Image Content Description]\n" + vlm_text
                else:
                    page_text = raw_text
            else:
                page_text = vlm_text or f"[Scanned page {page_num} contains no extractable digital text]"
        else:
            # Just extract text
            page_text = raw_text
...
def index_document(file_path, workspace_path=str(WORKSPACE_DIR)):
    ...
    if ext == ".pdf":
        ...
        pages = parse_pdf_hybrid(file_path, doc_id)
        ...
        # Run tree parser
        structure = _run_async(tree_parser(page_list, opt, doc=file_path, logger=pi_logger))
        ...
        pages, structure = parse_image(file_path, doc_id)
